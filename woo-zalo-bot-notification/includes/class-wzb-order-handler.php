...
class WZB_Order_Handler {
    ...
    public static function send_order_notification($order_id, $order, $settings) {
        ...
        foreach ($chat_ids as $chat_id) {
            ...
            if ($result['success']) {
                ...
                error_log('[WZB] Failed to send to ' . $chat_id . ': ' . $result['message']);
            }
        }
    ...
    public static function get_order_replacements($order, $settings) {
        ...
        $data['{order_total}'] = html_entity_decode(strip_tags($order->get_formatted_order_total()), ENT_QUOTES, 'UTF-8');
        ...
        $data['{full_address}'] = strip_tags(str_replace(array('<br>', '<br/>', '<br />'), ', ', $raw_address));
    ...
    private static function get_formatted_product_list($order) {
        ...
        foreach ($items as $item) {
            ...
            $total = html_entity_decode(strip_tags(wc_price($item->get_total(), array('currency' => $order->get_currency()))), ENT_QUOTES, 'UTF-8');
